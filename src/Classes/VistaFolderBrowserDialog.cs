...
public class VistaFolderBrowserDialog
{
    ...
    {
        this._selectedFolders = new List<string>();
    }
    ...
    {
        get => SelectedFolders.Count != 0 ? SelectedFolders[0] : string.Empty;
    }

    /// <summary>
    /// Gets selected folders when AllowMultiSelect is true.
    /// </summary>
    public IReadOnlyList<string> SelectedFolders => this._selectedFolders;
    ...
    /// <returns>
    /// <see langword="true"/> if the user clicks OK in the dialog box;
    ...
    private bool ShowVistaDialog(Window owner)
    {
        var platformHandle = owner.TryGetPlatformHandle();
        if (platformHandle == null) throw new InvalidOperationException();
        ...
        if (this.InitialFolder != null)
        {
            ...
            if (PInvoke.SHCreateItemFromParsingName(this.InitialFolder, null, in riid, out var obj_directoryShellItem).Value == NativeMethods.S_OK)
            {
                var directoryShellItem = Unsafe.As<NativeMethods.IShellItem>(obj_directoryShellItem);
        ...
        if (this.DefaultFolder != null)
        {
            ...
            if (/* NativeMethods.SHCreateItemFromParsingName
               (this.DefaultFolder, IntPtr.Zero, ref riid,
                ...
                PInvoke.SHCreateItemFromParsingName(this.InitialFolder, null, in riid, out var obj_directoryShellItem).Value == NativeMethods.S_OK)
            {
                var directoryShellItem = Unsafe.As<NativeMethods.IShellItem>(obj_directoryShellItem);
        ...
        if (frm.Show(platformHandle.Handle) == NativeMethods.S_OK)
        {
            if (AllowMultiSelect)
            {
                ...
                for (uint i = 0; i < numFolders; i++)
                {
                    ...
                    out IntPtr pszString) == NativeMethods.S_OK)
                    {
                        if (pszString != IntPtr.Zero)
                        {
                            try
                            {
                                var selectedItem = Marshal.PtrToStringAuto(pszString);
                                if (!string.IsNullOrEmpty(selectedItem))
                                {
                                    this._selectedFolders.Add(selectedItem);
                                }
                            }
                            finally
                            {
    ...
    /// Dispose the object
    /// </summary>
    public void Dispose() { this._selectedFolders.Clear(); } //just to have possibility of Using statement.
